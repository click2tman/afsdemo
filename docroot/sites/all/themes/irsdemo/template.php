 ...
 */
function irsdemo_preprocess_region(&$vars) {
  global $language;

  switch($vars['region']) {
    // menu region
    case 'menu':
      $dropdown_menu = menu_tree_output(_irsdemo_menu_build_tree('main-menu', array('max_depth'=>2)));
    ...
    case 'footer_first':
      $footer_menu = menu_tree_output(_irsdemo_menu_build_tree('main-menu', array('max_depth'=>2)));
      //set the active trail
      $active_trail = menu_get_active_trail();
      foreach($active_trail as $trail) {
 ...
 */
function irsdemo_menu_tree($variables) {
 ...
 */
function irsdemo_preprocess_views_view_unformatted(&$vars) {
 ...
 */
function _irsdemo_menu_build_tree($menu_name, $parameters = array()) {
 ...
 */
function irsdemo_preprocess_node(&$vars) {
  ...
  if ($vars['node']->type == 'profile' && $vars['view_mode'] == 'editors_choice') {
    ...
    if (isset($vars['content']['field_profile_social_media'][0])) {
      ...
      $vars['content']['field_profile_social_media'][0]['#markup'] = irsdemo_social_media_link($link);
 ...
 */
function irsdemo_social_media_link($link) {
 ...
 */
function irsdemo_preprocess_field(&$vars) {
 ...
 */
function irsdemo_preprocess_comment_wrapper(&$vars) {
  $vars['login_link'] = '';
 ...
 */
function irsdemo_reply_comments_wrap($element) {
 ...
 */
function irsdemo_preprocess_views_view_field(&$vars, $hook) {
 ...
 */
function irsdemo_preprocess_file_link(&$vars) {
...
function irsdemo_form_alter(&$form, &$form_state, $form_id) {
  if ($form_id == 'search_block_form') {
    ...
    $form['search_block_form']['#attributes']['placeholder'] = t('Enter keyword Demo...');
  }
}